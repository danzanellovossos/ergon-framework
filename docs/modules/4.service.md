...
Services are responsible for preserving the external system’s definition of atomicity. If a system emits micro-batches, a service retrieves them intact. If a system emits continuous streams, a service determines where atomic boundaries occur.
...
**Implementation Pattern**: Services should preferably use the framework's execution primitives (such as `run_fn`) to wrap external API calls and operations. This ensures that service methods automatically benefit from:

- Configurable retry policies with exponential backoff
- Timeout enforcement
- Automatic OpenTelemetry tracing and context propagation
- Consistent error handling and categorization

By leveraging these primitives, service implementations remain focused on protocol-specific logic while delegating resilience and observability concerns to the framework. This pattern ensures that all service operations are automatically instrumented and follow the same retry and timeout semantics as the rest of the framework.

Example pattern (language-agnostic concept):

```
Service {
    @run_fn(retry=retry_policy, trace_name="Service.operation")
    operation() {
        // Protocol-specific logic
        // Automatic retries, timeouts, and tracing applied
    }
}
```

This approach guarantees that services integrate seamlessly with the framework's observability and resilience mechanisms without requiring manual instrumentation or custom retry implementations.
