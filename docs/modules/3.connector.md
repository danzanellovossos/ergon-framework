...
Consumes inbox activity and sends messages via `channels.send`. Configure one inbox email; the connector resolves UUIDs internally. Fetch defaults to `pending_only=true`. `ack_transaction` / `nack_transaction` call the platform consumption endpoints (`channels:activity:consume`).
