...
Authentication and mailbox connection use [Nylas Hosted Auth](https://developer.nylas.com/docs/v3/auth/). Register your `redirect_uri` in the Nylas Dashboard before starting OAuth.

Executable examples: [`sdks/python/examples/nylas/`](../../sdks/python/examples/nylas/)

#### Authentication (obtain grant_id)

Use `NylasAuthService` to generate the Hosted OAuth URL and exchange the authorization code for a `grant_id`:
...
auth = NylasAuthService(
    NylasAuthClient(
        ...
        client_id="<NYLAS_CLIENT_ID>",  # optional; defaults to api_key
    )
)
...
# Step 2: after redirect, exchange the code for grant_id
...
Async variant: `AsyncNylasAuthService` with the same API.

CLI example: `python examples/nylas/auth_flow.py --generate-url` then `--exchange-code "..."`.

After obtaining `grant_id`, pass it to `NylasClient` for email connector operations.

#### Async Nylas Connector (recommended)
...
Use `service.list_folders()` to discover folder IDs for the `in_` filter.

#### Fetch → process → ack flow

1. **Fetch**: `fetch_transactions_async` returns `Transaction` objects with message payload and metadata (`thread_id`, `attachments`, `unread`).
2. **Process**: Task logic runs on each transaction.
3. **Ack**: `ack_transaction` applies `AckActionConfig` (mark read, move folder, star).

`nack_transaction` is a no-op for Nylas — messages remain available for refetch.

#### Dispatch (send email)

Set `producer_config.send_mode` to `"send"` for direct send or `"draft"` to create and send a draft. The transaction `payload` should be a `SendMessageInput` or a dict compatible with the Nylas send API.

#### Sync Nylas Connector

```python
from ergon.connector.nylas import NylasConnector
...
- `subject` filter at the API level is case-sensitive; use `client_side_filter.subject_contains` for flexible matching.
- `search_query_native` cannot be combined with `unread`, `has_attachment`, `subject`, etc. (only `in`, `limit`, and `page_token` are allowed alongside it). For attachment filename filtering that composes with other filters, use `client_side_filter.attachment_filename_contains`.
- `search_query_native` syntax varies by provider (Google, Microsoft, IMAP).
- Large Microsoft attachments (>25MB) require Nylas large-attachment flow (not included in this connector).
