...
#### Multi-subscription consumers

One connector can consume from multiple queues and bind each queue to one or
more exchanges. This keeps transport fan-in inside the RabbitMQ
Service/Connector boundary: Tasks still receive ordinary `Transaction`
objects and do not know how many queues feed them.
...
consumer_config = AsyncRabbitmqConsumerConfig(
    subscriptions=[
        ...
        AsyncRabbitmqQueueSubscription(
            queue_name="audit.conversations",
            bindings=[
                AsyncRabbitmqExchangeBinding(
                ...
                AsyncRabbitmqExchangeBinding(
                    exchange_name="iam",
                    routing_keys=["iam.service_access.granted"],
                ),
            ],
        ),
    ],
...
The legacy `queue_name` / `exchange_name` / `binding_keys` fields remain
supported and normalize to one subscription. Messages include
`queue_name` and `exchange_name` in `Transaction.metadata`, allowing domain
handlers to inspect their source without depending on aio-pika objects.

#### Liveness and bounded recovery

Connection establishment, robust reconnection, and channel creation are
bounded by `AsyncRabbitmqClient.connect_timeout` and `channel_timeout`.
Consumer health distinguishes message activity from loop progress:

- `last_poll_completed_ts` advances after successful empty polls, so an idle
  queue remains healthy.
- `last_fetch_ts` records actual message delivery.
- `in_flight_count` records delivered but unsettled messages.
- `state` and `consumer_epoch` expose reconnect and subscription rebuilds.

Async consumer Tasks evaluate these transport signals together with
transaction progress. The Runner supervises that generic Task liveness from an
independent thread and requests process shutdown after sustained failure,
allowing the external orchestrator to replace a wedged worker.
