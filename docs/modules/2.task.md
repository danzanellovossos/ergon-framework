...
- **TaskMeta**: a shared metaclass that injects common lifecycle and initialization behavior into both synchronous and asynchronous base task classes.
...
-   **Synchronous**: Executed through a thread-isolated `future.result(timeout=...)`.
-   **Asynchronous**: Wrapped in `asyncio.wait_for`.
...
-   **Synchronous**: Uses a `ThreadPoolExecutor` with configurable concurrency; each transaction is processed in its own worker thread, guaranteeing isolation from other workloads running in the same consumer loop.
-   **Asynchronous**: Uses an `asyncio.Semaphore` to enforce concurrency limits, enabling highly parallel, high-throughput pipelines without blocking the event loop.
