...
- **Process Phase**: Wraps the user-defined business logic (`process_transaction`). Failures are retried according to the process policy unless user code raises a non-retryable signal (see Execution Primitives), which immediately transitions execution into the exception handler without further attempts.
...
The Task Module supports three distinct execution modes. All modes operate on the same task abstraction, enforce the same lifecycle guarantees, and provide identical observability instrumentation. The difference lies in the scope and granularity of execution.

#### Full Task Execution

Full task execution represents running a task as a complete, self-contained execution unit. In this mode, the task owns its internal loop—whether consumer, producer, or hybrid—and manages the continuous processing of transactions according to its configured policies.

Execution may be finite or long-running. A finite task processes a bounded set of transactions and terminates; a long-running task operates in streaming mode, continuously polling for new work until explicitly stopped. Shutdown is cooperative and lifecycle-aware, ensuring that in-flight transactions complete gracefully and all resources are released cleanly.

The unit of responsibility in full task execution is the task itself, not individual transactions. The framework orchestrates batching, concurrency, retries, backoff, and telemetry across the entire execution lifecycle.

#### Single Transaction Execution

Single transaction execution processes exactly one transaction through the task lifecycle. There is no consumer loop, no batching, and no streaming behavior. The transaction is provided directly to the task, which applies the same processing, success, and exception semantics as full execution.

This mode guarantees atomicity and observability for a single unit of work. The transaction passes through the same lifecycle phases—process, success handler, exception handler—with the same retry policies and timeout enforcement. The only difference is scope: execution begins with one transaction and ends when that transaction's lifecycle completes.

Single transaction execution is intended for deterministic, isolated execution scenarios where the goal is to process a known transaction with full lifecycle guarantees.

#### Transaction-by-Identifier Execution

Transaction-by-identifier execution is a specialization of single transaction execution. Rather than receiving the transaction directly, the framework resolves a transaction by its identifier through a connector. Once resolved, the transaction is processed exactly once using the same lifecycle as single transaction execution.

All lifecycle guarantees remain unchanged. The resolved transaction receives identical treatment: structured retries, timeout enforcement, success and exception handling, and full observability. The only additional step is the initial resolution phase, where the framework retrieves the transaction from its source.

This mode enables targeted reprocessing and recovery scenarios. When a specific transaction requires re-execution—whether for debugging, recovery, or operational intervention—the framework can retrieve and process it in isolation without affecting other transactions or requiring a full task execution cycle.
...
**Non-Retryable Signal**: `run_fn` expects SDKs to implement a non-retryable exception type. When this signal is raised, `run_fn` immediately stops retrying and returns failure—regardless of the configured retry policy. This signal is orthogonal to domain exception categories (BUSINESS, SYSTEM, TIMEOUT); retryability is not inferred from what kind of error occurred, but from whether user code explicitly signals that retrying is futile. This design keeps execution primitives generic while giving user code explicit control over retry behavior.
...
Async consumer mixins expose a transport-agnostic liveness snapshot to the
Runner. The snapshot combines consumer-loop progress, in-flight transaction
state, and optional Connector health. It deliberately treats message delivery
and acknowledgement timestamps as activity diagnostics rather than liveness:
an empty queue may have no recent delivery or acknowledgement while polling
normally.

`TaskConfig.supervision` controls process-level reaction:

- `startup_grace`: time allowed for initialization.
- `check_interval`: liveness evaluation cadence.
- `unhealthy_grace`: sustained failure required before shutdown is requested.
- `shutdown_grace`: time allowed for cooperative cancellation before the
  supervisor forces a non-zero process exit.
- `fetch_stale_after` and `processing_stale_after`: optional overrides for
  legitimate long-polling or long-running transaction workloads; otherwise
  thresholds are derived from Connector and transaction policies.

The supervisor runs in a native thread so it remains able to terminate a
worker when the asyncio event loop itself is wedged. It only understands the
generic `LivenessProvider` contract; protocol-specific state remains in
