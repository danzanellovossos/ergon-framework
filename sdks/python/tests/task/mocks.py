...
from ergon.connector import AsyncConnector, Connector, Transaction
from ergon.task.mixins.consumer import AsyncConsumerMixin, ConsumerMixin
from ergon.task.mixins.producer import AsyncProducerMixin, ProducerMixin
...
class MockAsyncConnector(AsyncConnector):
    """In-memory async connector that pops items from an internal queue."""

    def __init__(self, transactions: List[Transaction] | None = None):
        self._queue: List[Transaction] = list(transactions or [])

    async def fetch_transactions_async(self, batch_size: int = 1, **kwargs) -> List[Transaction]:
        batch = self._queue[:batch_size]
        self._queue = self._queue[batch_size:]
        return batch

    async def dispatch_transactions_async(self, transactions: List[Transaction], **kwargs) -> Any:
        pass


class MockAsyncConsumer(AsyncConsumerMixin):
    """
    Concrete AsyncConsumerMixin that bypasses BaseAsyncTask/TaskMeta.
    ...
    async def process_transaction(self, transaction: Transaction) -> Any:
        self.processed.append(transaction)
        if self._process_fn:
            return await self._process_fn(transaction)
        return transaction.payload

    async def handle_process_success(self, transaction, result):
        if self._success_fn:
            return await self._success_fn(transaction, result)
        return await super().handle_process_success(transaction, result)

    async def handle_process_exception(self, transaction, exc):
        if self._exception_fn:
            return await self._exception_fn(transaction, exc)
        return await super().handle_process_exception(transaction, exc)


class MockAsyncProducer(AsyncProducerMixin):
    """
    Concrete AsyncProducerMixin that bypasses BaseAsyncTask/TaskMeta.
    ...
    def __init__(
        ...
        self.name = "test-async-producer"
        self.prepared: List[Transaction] = []
        self._prepare_fn = prepare_fn
        self._success_fn = success_fn
        self._exception_fn = exception_fn

    async def prepare_transaction(self, transaction: Transaction) -> Any:
        self.prepared.append(transaction)
        if self._prepare_fn:
            return await self._prepare_fn(transaction)
        return transaction.payload

    async def handle_prepare_success(self, transaction, result):
        if self._success_fn:
            return await self._success_fn(transaction, result)
        return await super().handle_prepare_success(transaction, result)

    async def handle_prepare_exception(self, transaction, exc):
        if self._exception_fn:
            return await self._exception_fn(transaction, exc)
        return await super().handle_prepare_exception(transaction, exc)
