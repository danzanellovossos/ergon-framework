...
class InboxAddressBook:
    """Looks up ``ResolvedInboxAddress`` rows from addresses, config, or activity."""
    ...
    def resolve_consumer(self, config: ErgonPlatformChannelsConsumerConfig) -> ResolvedInboxAddress:
        """Resolve an inbox address from a consumer config."""
        if config.config_id:
    ...
    def resolve_sender(
        ...
        """Resolve an inbox address by email or id."""
        if address_id:
            try:
        ...
        if address and config_id:
            try:
                return self.resolve_in_config(config_id, address, direction_override=direction)
            except ValueError:
                pass
        if address:
    ...
    def resolve_email(self, address_email: str) -> ResolvedInboxAddress:
        """Resolve an inbox address by email."""
    ...
    def resolve_id(self, address_id: str) -> ResolvedInboxAddress:
        """Resolve an inbox address by id."""
    ...
    def resolve_in_config(
        ...
        """Resolve an inbox address in a config."""
        if address_id:
        ...
        try:
            ...
            for entry in SdkRecord.items(page, keys=["items", "data", "results"]):
                parsed = self._parse_config_entry(entry, config_id=config_id)
        ...
        return self._remember(
            self._from_config_activity(config_id, address_email, direction_override=direction_override)
        )

    def _remember(self, resolved: ResolvedInboxAddress) -> ResolvedInboxAddress:
        """Remember a resolved inbox address."""
    ...
    def _with_config_capabilities(self, inbox: ResolvedInboxAddress) -> ResolvedInboxAddress:
        """Add config capabilities to a resolved inbox address."""
    ...
    def _from_config_activity(
        ...
        """Resolve an inbox address from config activity."""
        ...
        for item in SdkRecord.items(created_response, keys=["items", "data", "results"]):
            resolved = self._match_activity_item(
                config_id, address_email, normalized, item, direction_override=direction_override
            )
        ...
        for _ in range(5):
            ...
            items = SdkRecord.items(response, keys=["items", "data", "results"])
            if not items:
                break
            for item in items:
                resolved = self._match_activity_item(
    ...
    def _match_activity_item(
        ...
        """Match an activity item to an inbox address."""
        ...
        detail = self._client.channels.configs.activity_event(config_id, str(event_id))
    ...
    def _load_granted_addresses(self) -> None:
        """Load granted addresses from the platform."""
        if self._by_email:
            return
        for entry in SdkRecord.items(self._client.channels.addresses(), keys=["items", "data", "results"]):
    ...
    @staticmethod
    def _parse_granted_entry(entry: Any) -> Optional[ResolvedInboxAddress]:
        """Parse a granted entry into a ``ResolvedInboxAddress``."""
        record = SdkRecord(entry)
        entry_email = record.get("address")
        entry_id = record.get("id")
        entry_cfg = record.get("channel_config_id")
